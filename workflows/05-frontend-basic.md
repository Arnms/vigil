...
**목표**: 새로운 엔드포인트 등록 및 기존 엔드포인트 수정 폼 구현
**상태**: ✅ 완료
...
- [x] 폼 필드 ✅
  - 이름 (텍스트 입력) ✅
  - URL (텍스트 입력, URL 유효성 검사) ✅
  - HTTP 메소드 (드롭다운) ✅
  - 헤더 (JSON 텍스트 영역) ✅
  - 바디 (JSON 텍스트 영역) ✅
  - 체크 간격 (드롭다운: 30초-1시간) ✅
  - 예상 응답 코드 (드롭다운: 2xx/3xx/4xx/5xx) ✅
  - 타임아웃 임계값 (숫자 입력, 1000ms 이상) ✅
  - 활성화 여부 (체크박스) ✅

- [x] 폼 검증 ✅
  - 필수 필드 확인 ✅
  - URL 형식 검증 (new URL 사용) ✅
  - 숫자 범위 검증 (체크 간격, 상태 코드, 타임아웃) ✅
  - 실시간 에러 제거 ✅

- [x] 제출 기능 ✅
  - API 호출 (createEndpoint, updateEndpoint) ✅
  - 성공/실패 처리 ✅
  - 토스트 알림 ✅
  - 제출 중 버튼 비활성화 ✅
...
- [x] 추가 컴포넌트 ✅
  - `src/components/InfoCard.tsx` - 통계 정보 카드 ✅
  - `src/components/CheckResultsList.tsx` - 체크 결과 목록 ✅
  - `src/components/IncidentsList.tsx` - 인시던트 목록 ✅
